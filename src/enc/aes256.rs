...
pub async fn aes256_enc(key_str_base64: &String, iv_str_base64: &String, plaintext: &String) -> String {
    ...
    let ct = Aes256CbcEnc::new(&key.into(), &iv.into()).encrypt_padded_mut::<Pkcs7>(&mut buf, pt_len).unwrap();
    let ct_base64 = BASE64_STANDARD.encode(&ct);
    ct_base64.to_string()
}

pub async fn aes256_dec(key_str_base64: &String, iv_str_base64: &String, encrypted_text_base64: &String) -> String {
    let key_decoded: Vec<u8> = BASE64_STANDARD.decode(key_str_base64.as_bytes()).unwrap();
    let key: [u8; 32] = key_decoded.try_into().unwrap();
    ...
    let encrypted_text_decoded: Vec<u8> = BASE64_STANDARD.decode(encrypted_text_base64.as_bytes()).unwrap();
